...
namespace Iot.Device.Max7219
{
    ...
    public partial class Max7219 : IDisposable
    {
        ...
        /// <summary>
        /// Rotates the full display, not only single digits. 
        /// This can be used if there is no possibility to rotate the physical display
        /// </summary>
        public bool RotateFullDisplay { get; set; }

        /// <summary>
        /// Creates a Max7219 Device given a <see paramref="spiDevice" /> to communicate over and the
        /// number of devices that are cascaded.
        /// </summary>
        public Max7219(SpiDevice spiDevice, int cascadedDevices = 1, RotationType rotation = RotationType.None, bool rotateFullDisplay = false)
        {
            _spiDevice = spiDevice ?? throw new ArgumentNullException(nameof(spiDevice));
            CascadedDevices = cascadedDevices;
            Rotation = rotation;
            RotateFullDisplay = rotateFullDisplay;
            _buffer = new byte[CascadedDevices][];
            for (int i = 0; i < CascadedDevices; i++)
            {
                _buffer[i] = new byte[NumDigits];
            }
        ...
        /// and digit position
        /// </summary>
        public byte this[DeviceIdDigit device]
        {
            get
            {
                ValidatePosition(device.DeviceId, device.Digit);
                return _buffer[device.DeviceId][device.Digit];
            }
            set
            {
                ValidatePosition(device.DeviceId, device.Digit);
                _buffer[device.DeviceId][device.Digit] = value;
            }
        }
        ...
        private void ValidateIndex(int index, out int deviceId, out int digit)
        {
            ...
            // Orignial code uses Math.DivRem
            deviceId = index / NumDigits;
            digit = index - deviceId * NumDigits;
        }
        ...
        private void WriteBufferWithoutRotation(byte[][] buffer)
        {
            for (int digit = 0; digit < NumDigits; digit++)
            {
                ...
                {
                    for (var deviceId = 0; deviceId <= CascadedDevices - 1; deviceId++)
                    {
                        _writeBuffer[i++] = (byte)((int)Register.DIGIT0 + digit);
                        _writeBuffer[i++] = buffer[deviceId][digit];
                    }
        ...
        public void Clear(int start, int end, bool flush = true)
        {
            ...
            for (int deviceId = start; deviceId < end; deviceId++)
            {
                for (int digit = 0; digit < NumDigits; digit++)
                {
                    this[new DeviceIdDigit(deviceId, digit)] = 0;
                }
            }
